    ...
    function renderSpreadsheetGrid() {
      ...
      rows.forEach((r, rowNum) => {
        ...
        html += `<tr class="${bg} hover:bg-amber-50/40 dark:hover:bg-slate-800 transition border-b border-slate-200 dark:border-slate-700/60">
          ...
          <td class="py-1 px-1 border-r border-slate-200 dark:border-slate-700/60">
            <textarea onfocus="selectExcelCell('DESC_${displayRow}', this); this.style.height='auto'; this.style.height=this.scrollHeight+'px'; this.style.whiteSpace='normal';" onblur="this.style.height=''; this.style.whiteSpace='nowrap';" oninput="this.style.height='auto'; this.style.height=this.scrollHeight+'px';" onchange="onSheetCellChange(${idx}, 'desc', this.value)" placeholder="Enter course description..." rows="1" class="w-full px-1.5 py-0.5 text-xs text-slate-700 dark:text-slate-300 bg-transparent hover:bg-white dark:hover:bg-slate-800 focus:bg-white dark:focus:bg-slate-800 border-0 rounded-none focus:outline-none resize-none overflow-hidden whitespace-nowrap leading-tight">${(c.desc || '').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</textarea>
          </td>
