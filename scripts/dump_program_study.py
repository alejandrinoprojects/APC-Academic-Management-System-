...
with zipfile.ZipFile(wb_path, 'r') as z:
    ...
    # Dump Program Study (Sheet 4) courses: Code, Title, Units, Lec, Lab, Prereq, Co-req
    for name, sId, rId in sheets_info:
        if name in ['Program Study', 'Summary of Units']:
            ...
            for r in rows:
                ...
                for c in r.findall('{http://schemas.openxmlformats.org/spreadsheetml/2006/main}c'):
                    ...
                    cells[col] = val
                if any(cells.values()):
                    line = " | ".join([f"{k}:{v}" for k,v in sorted(cells.items()) if v])
                    if len(line) > 160:
                        line = line[:160] + '...'
                    print(f"R{r.attrib.get('r')}: {line}")
